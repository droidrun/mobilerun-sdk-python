...
class CatalogResource(SyncAPIResource):
    ...
    *,
    ...
    timeout: float | httpx.Timeout | None | NotGiven = not_given,) -> CatalogRetrieveResponse:
        """
        ...
        if not app_event_type:
          raise ValueError(
            f'Expected a non-empty value for `app_event_type` but received {app_event_type!r}'
          )
        return self._get(
    ...
    *,
    ...
    extra_query: Query | None = None,
    extra_body: Body | None = None,
    timeout: float | httpx.Timeout | None | NotGiven = not_given,) -> CatalogListResponse:
