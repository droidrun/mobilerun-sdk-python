# Droidrun Cloud Python API library

<!-- prettier-ignore -->
[![PyPI version](https://img.shields.io/pypi/v/droidrun_cloud.svg?label=pypi%20(stable))](https://pypi.org/project/droidrun_cloud/)

The Droidrun Cloud Python library provides convenient access to the Droidrun Cloud REST API from any Python 3.9+
...
pip install git+ssh://git@github.com/stainless-sdks/droidrun-cloud-python.git
```
...
to add `DROIDRUN_CLOUD_API_KEY="My API Key"` to your `.env` file
so that your API Key is not stored in source control.

## Async usage

Simply import `AsyncDroidrunCloud` instead of `DroidrunCloud` and use `await` with each API call:

```python
import os
import asyncio
from droidrun_cloud import AsyncDroidrunCloud
...
```sh
# install from this staging repo
pip install 'droidrun_cloud[aiohttp] @ git+ssh://git@github.com/stainless-sdks/droidrun-cloud-python.git'
```
...
client.with_options(max_retries=5).apps.list()
```
...
client.with_options(timeout=5.0).apps.list()
```
...
You can enable logging by setting the environment variable `DROIDRUN_CLOUD_LOG` to `info`.

```shell
$ export DROIDRUN_CLOUD_LOG=info
```
...
app = response.parse()  # get the object that `apps.list()` would have returned
print(app.apps)
```
...
The async client returns an [`AsyncAPIResponse`](https://github.com/stainless-sdks/droidrun-cloud-python/tree/main/src/droidrun_cloud/_response.py) with the same structure, the only difference being `await`able methods for reading the response content.
...
```python
import httpx
from droidrun_cloud import DroidrunCloud, DefaultHttpxClient

client = DroidrunCloud(
    # Or use the `DROIDRUN_CLOUD_BASE_URL` env var
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/droidrun-cloud-python/issues) with questions, bugs, or suggestions.
